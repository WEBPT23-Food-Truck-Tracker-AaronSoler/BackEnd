...
const addTruckRating = async (userId, body) => {
	const [id] = await db('diners_truck_ratings').insert(
		{ rating: body.rating, diner_id: userId, truck_id: body.truck_id },
		'id'
	);
...
const addMenuItemRating = async (userId, body) => {
	const [id] = await db('diners_menu_items_ratings').insert(
		{ rating: body.rating, diner_id: userId, menu_item_id: body.menu_item_id },
		'id'
	);
...
const getFavoriteTrucks = (userId) => {
	return db('diner_favorite_trucks')
		.join('trucks', 'diner_favorite_trucks.truck_id', 'trucks.id')
		.join('diners', 'diner_favorite_trucks.diner_id', 'diners.id')
		.where('diners.id', userId)
		.select('trucks.*');
};

const addFavoriteTruck = async (truckId, dinerId) => {
	const [id] = await db('diner_favorite_trucks').insert(
		{ truck_id: truckId, diner_id: dinerId },
		'id'
	);
